...
import math
import flow_matching.flow_and_diffusion.edges2shoes.mnitsutils as util
...
class ResidualLayer(nn.Module):
    def __init__(self, channels, time_embed_dim):
        ...
        self.cond_adapter = nn.Conv2d(channels, channels, 1)  # project cond feat to match channels

    def forward(self, x, t_embed, cond_feat):
        res = x
        x = self.block1(x)
        t = self.time_adapter(t_embed).unsqueeze(-1).unsqueeze(-1)
        x = x + t
        x = x + self.cond_adapter(cond_feat)  # spatially aligned add
        x = self.block2(x)
        return x + res
...
class CondEncoder(nn.Module):
    def __init__(self, cond_in_ch: int, feats: List[int]):  # feats aligns with U-Net channels
        super().__init__()
        self.stem = nn.Conv2d(cond_in_ch, feats[0], 3, padding=1)
        self.downs = nn.ModuleList([
            nn.Sequential(
                nn.SiLU(), nn.Conv2d(c_in, c_out, 3, padding=1),
                nn.SiLU(), nn.Conv2d(c_out, c_out, 3, padding=1),
                nn.Conv2d(c_out, c_out, 3, stride=2, padding=1)
            )
            for c_in, c_out in zip(feats[:-1], feats[1:])
        ])
    def forward(self, c):           # c: (bs, cond_in_ch, H, W)
        feats = []
        x = self.stem(c)
        feats.append(x)             # scale 0
        for down in self.downs:
            x = down(x)
            feats.append(x)         # scales 1..N
        return feats                # list of (bs, ch_i, H_i, W_i)
...
class MNISTUNet(util.ConditionalVectorField):
    def __init__(self, channels: List[int], num_residual_layers: int, t_embed_dim: int, y_embed_dim: int):
        ...
        for (curr_c, next_c) in zip(channels[:-1], channels[1:]):
            encoders.append(CondEncoder(curr_c, next_c, num_residual_layers, t_embed_dim, y_embed_dim))
